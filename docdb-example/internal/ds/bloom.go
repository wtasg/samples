// Package ds implements the core data structures used by the DocDB engine.
//
// bloom.go — Bloom Filter used as a per-SST existence gate in the LSM Tree.
//
...
//     is definitely absent, so we can skip the SST binary search entirely.
//   - False positives are possible: MightContain may return true for a key
//     that is not actually present; we must then confirm via SST lookup.
//
...
// BloomFilter gates key existence checks per SSTable.
...
// hashPositions returns k bit positions for the given string key using
// double hashing: pos_i = (h1 + i*h2) mod m.
func (bf *BloomFilter) hashPositions(key string) []uint {
	b := []byte(key)
...
// hashPositionsInt returns k bit positions for an int64 key.
func (bf *BloomFilter) hashPositionsInt(key int64) []uint {
	b := make([]byte, 8)
...
// AddInt records that an int64 key is a member of the set.
func (bf *BloomFilter) AddInt(key int64) {
	for _, p := range bf.hashPositionsInt(key) {
		bf.bits[p/64] |= 1 << (p % 64)
	}
}
...
// MightContainInt returns false if an int64 key is DEFINITELY absent.
func (bf *BloomFilter) MightContainInt(key int64) bool {
	for _, p := range bf.hashPositionsInt(key) {
		if bf.bits[p/64]&(1<<(p%64)) == 0 {
			return false
		}
...
// Reset clears all bits (used when compacting SSTables).
...
// the number of bits set. Formula: (setBits/m)^k
...
func (bf *BloomFilter) Bits() []uint64 { return bf.bits }

// M returns the total bit count.
