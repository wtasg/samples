...
function detectProtocol() {
  try {
    ...
    if (protocol) {
      ...
      if (protocol.includes('h3') || protocol.includes('http3')) {
        activeProtocol.className = 'protocol-badge http3';
        logEvent('system', `Detected communication protocol: HTTP/3 (${displayProto})`);
      } else {
...
function drawChart() {
  ...
  areaGradient.addColorStop(0, 'rgba(175, 82, 222, 0.15)');
  areaGradient.addColorStop(1, 'rgba(175, 82, 222, 0.0)');
  ...
  ctx.lineWidth = 3;
  ctx.shadowColor = 'rgba(175, 82, 222, 0.3)';
  ctx.shadowBlur = 8;
  ctx.stroke();
